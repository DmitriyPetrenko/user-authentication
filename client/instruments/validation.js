...
export const validateFieldOfForm = {
    username(content) {
        ...
        if (content.length >= minLength.username && content.length <= maxLength.username) {
            return {
                isValid: true,
                messageError: ''
            };
        } else if (content.length < minLength.username && content.length !== 0) {
            return {
                isValid: false,
    ...
    password(content) {
        ...
        if (content.length >= minLength.password && content.length <= maxLength.password && find.letter && find.digit) {
            return {
                isValid: true,
                messageError: ''
            };
        } else if (content.length < minLength.password && content.length !== 0) {
            return {
                isValid: false,
